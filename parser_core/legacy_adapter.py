...
class LegacyAdapter:
    ...
    All ``collect_urls`` and ``parse_business`` calls are serialised via a
    class-level lock because the legacy modules store mutable configuration
    in module-level globals that cannot be safely shared across threads.
    """
